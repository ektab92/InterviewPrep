...
public class LongestPathInDAG {
    ...
    public Map<Integer, List<Edge>> createGraph(int graph[][], int vertices)
    {
        ...
        {
            //make wgt negetive
            Edge e = new Edge(graph[i][0],graph[i][1],-1*graph[i][2]);
    ...
    public int[] findLongestPath(Map<Integer, List<Edge>> adjList,int N)
    {
        int [] dist = new int[N];
    ...
    public static void main(String[] args) {
        int [][]Graph = {{0,1,4},{0,7,8},{1,7,11},{1,2,8},{7,8,7},{7,6,1},
                ...
                {5,4,10}};
        LongestPathInDAG dag = new LongestPathInDAG();
        Map<Integer, List<Edge>> adjList = dag.createGraph(Graph,9);
        dag.findLongestPath(adjList,9);
    }
}
